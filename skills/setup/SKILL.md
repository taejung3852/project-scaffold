...
프로젝트 시작 시 실행. 2~3시간 철저한 인터뷰를 통해 팀 컨벤션을 정의하고
`wiki/conventions/`와 `AGENT.md`를 자동 생성한다.

---

## Step 0 — 기존 설정 확인

`wiki/conventions/` 디렉토리가 존재하는지 확인한다.
...
모든 13개 카테고리 완료 후 `AGENT.md`를 생성한다.
...
| 카테고리 순서 | 1→13 순서 고정. 건너뛰기 불가 |
| AGENT.md 생성 | 13개 전부 완료 후에만 생성 |
