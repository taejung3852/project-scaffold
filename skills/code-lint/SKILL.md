...
도구가 설치되지 않은 경우: 조용히 건너뛰지 않는다. 설치 명령어를 안내하고 설치할지 물어본다.
- 설치를 원하면: 안내한 명령어로 설치 후 그 도구로 검증을 이어간다
- 원치 않으면: 그 도구만 "미설치(스킵)"로 표시하고 나머지는 계속 진행
...
| 정적 도구 | 미설치 시 설치 명령어 안내 + 설치 여부 확인. 동의 없이 임의 설치 금지 |
| 자동 수정 | 금지. 제안만 함 |
| HITL | 🔴 위반 시 반드시 사람 확인 |
