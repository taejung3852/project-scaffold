...
| `/capture` | 회의·결정·개발 내용 기록 | 수시 |
| `/ingest` | raw/ 파일을 wiki에 반영 | capture 후 |
| `/query` | wiki 기반 질의응답 | 필요 시 |
| `/report` | 회의록·ADR·스프린트 요약 생성 | 필요 시 |
| `/code-lint` | 컨벤션 기반 코드 검증 | PR 전 |
| `/wiki-lint` | wiki 품질 점검 | 주기적 |
| `/dashboard` | 프로젝트 현황 확인 | 필요 시 |
| `/curate` | 스킬 정리·진화 (에이전트가 제안, 사용자가 실행 여부 결정) | 주기적 |
...
| 세션 시작 시 | 권고 섹션 읽고 유지보수 필요하면 한 줄 제안 | 에이전트 |
