...
```bash
pip install graphifyy       # 설치
graphify install            # AI 에이전트에 스킬 자동 등록 (Claude Code, Cursor 등)
```

`init.sh`가 graphifyy 설치 여부를 자동 감지해 등록까지 처리한다. 이후 wiki/를 갱신할 때마다 아래 명령으로 그래프를 업데이트한다:
