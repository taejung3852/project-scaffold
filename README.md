...
[NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent)는 스킬 기반 자가 진화 AI 에이전트 하네스다. 에이전트가 오래 쓰일수록 스킬이 누적되고 중복이 생기며 사용 패턴이 바뀐다. Hermes는 이 문제를 **4개의 Python 레이어**로 해결한다.
...
`.md` 파일로 작성된 스킬은 선언적이고 에이전트 무관하다는 장점이 있지만, 사용 추적·상태 전이·파일 이동 같은 결정론적 로직은 코드로 처리해야 한다. Hermes의 4-레이어 구조는 이 경계를 명확하게 분리한다 — **선언(`SKILL.md`)은 에이전트가, 판정(`curator.py`)은 Python이** 담당한다.

project-scaffold는 이 구조를 그대로 채택했다. 스킬을 많이 쓸수록 `skills/.usage.json`에 데이터가 쌓이고, `prompt_builder.py`가 `AGENT.md`의 권고 섹션을 자동 갱신하며, `/curate` 스킬이 LLM 판단으로 통합·정리한다.
...
[oh-my-openagent (OmO)](https://github.com/code-yeongyu/oh-my-openagent)는 복잡한 코드베이스를 위한 오픈소스 에이전트 오케스트레이션 하네스다. Multi-Model Orchestration, 특화 에이전트 팀(Sisyphus·Prometheus·Atlas·Oracle), MCP+LSP/AST 정적 분석으로 에이전트 생산성을 극대화한다.

| | oh-my-openagent (OmO) | project-scaffold |
|---|---|---|
| 방향 | **엑셀러레이터** — 에이전트에게 폭넓은 도구와 권한 | **가드레일** — 에이전트 오작동·규칙 위반 통제 |
| 대상 | 빠른 프로토타이핑 (1인·소규모팀) | 현업 협업팀 (Enterprise Guardrail) |
| 지식 관리 | 없음 | LLM Wiki (`wiki/` 레이어) |
| devlog | 없음 | 커밋마다 자동 생성 (post-commit 훅) |
| 컨벤션 강제 | 없음 | Hard(훅) + Soft(AGENT.md) 2-레이어 |
| 에이전트 무관 | 플랫폼 종속 | `AGENT.md` 단일 소스, `/switch-agent`로 즉시 전환 |

OmO가 "빠르게 만들기 위해 에이전트에게 최대한의 자유를"을 추구한다면, project-scaffold는 "팀이 AI와 같이 일할 때 일관성·기록·협업을 기계적으로 보장"에 집중한다. 두 도구는 경쟁 관계가 아니라 목적이 다르다.

---

## 시스템 구조

8개의 스킬, 4개의 Python 스크립트, 2개의 git hook, 그리고 `AGENT.md`·`SOUL.md` 이중 진실 소스로 구성된다.
...
`wiki/` 폴더를 [Obsidian](https://obsidian.md) vault로 바로 열 수 있다. `.obsidian/` 폴더가 사전 세팅되어 있어 clone 직후 아무 설정 없이 아래 기능이 즉시 동작한다.
...
[Graphify](https://graphify.net/)는 AI 코딩 어시스턴트를 위한 오픈소스 지식 그래프 스킬로, project-scaffold 하네스에 내장된다. Tree-sitter AST + LLM 의미 추출 + Leiden 클러스터링으로 wiki/와 코드를 통합 파싱해 쿼리 가능한 그래프를 생성한다.

`init.sh` 실행 후 `/graphify` 스킬이 자동으로 설정된다. `graphify-out/`은 로컬 생성 파일로 `.gitignore`에 포함되며, 개인이 자신의 코드베이스 맥락을 AI에게 효율적으로 전달하기 위한 도구다.

```bash
# wiki/ + 코드 전체를 지식 그래프로 빌드
/graphify ./wiki
...
| 코드와 문서의 연결 관계 파악 | wiki/ + 코드 통합 분석 → God Nodes·Surprise Edges 탐지 |
...
│   └── ideas/
│
├── .obsidian/                      ← Obsidian 설정 (폴더별 색상 그룹 미리 세팅)
│
...
| 항목 | 상태 |
|---|---|
| 스킬 파일 8개 (setup·capture·ingest·query·code-lint·wiki-lint·dashboard·curate) | ✅ 완료 |
...
| Graphify 통합 (`pip install graphifyy`) | 🔜 예정 |
