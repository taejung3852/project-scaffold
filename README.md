...
`init.sh`가 graphifyy 설치 여부를 자동 감지해 AI 에이전트 등록까지 처리한다. graphifyy가 없으면 아래 명령어를 안내해준다:

```bash
pip install graphifyy   # graphifyy 미설치 시 한 번만 실행
bash init.sh            # 재실행하면 자동 등록 완료
```
